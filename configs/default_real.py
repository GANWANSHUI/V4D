...
density_feat_size = 12            # channel size of the voxel
lut_field=True
...
data = dict(
    ...
    eval_lpips_vgg=True,  # D-NeRF 用的是vgg
    eval_mse=True,
...
coarse_train = dict(
    ...
    N_rand=8192,                  # batch size (number of random rays per optimization step), 6192  8192, 16384, 32768

    # lrate_density_1=1e-1,  # lr of density_feat voxel grid
    # lrate_k0_1=1e-1,  # lr of color/feature voxel grid

    lrate_density=1e-1,           # lr of density voxel grid
    lrate_density_feat=1e-1,      # lr of density_feat voxel grid
    lrate_k0=1e-1,                # lr of color/feature voxel grid
    lrate_rgbnet=1e-3,            # lr of the mlp to predict view-dependent color
    lrate_occupancy_net=1e-3,     # lr of the occupancy_net mlp to predict density
    lrate_flow_net = 1e-3,
...
fine_train.update(dict(
    N_iters=100000,
    pervoxel_lr=False,
    # ray_sampler='flatten', #  original is 'in_maskcache'
    weight_entropy_last=0.0,
    weight_rgbper=0.01,
    pg_scale=[10000, 20000, 30000, 40000],
    # pg_scale=[2000, 4000, 6000, 8000],
    # pg_scale=[5000, 10000, 15000, 20000],
    # pg_scale=[20000, 40000, 60000, 80000],

    tv_every=1,  # count total variation loss every tv_every step
    tv_from=1,  # count total variation loss from tv_from step
...
coarse_model_and_render = dict(
    geometry_search = False,       # geometry search for xyz_min_fine, xyz_max_fine
    ...
    alpha_init=1e-5,    #1e-6     # set the alpha values everywhere at the begin of training
