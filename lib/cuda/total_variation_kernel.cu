...
__global__ void total_variation_add_grad_cuda_kernel(
  ...
  if(index<N && (dense_mode || grad[index]!=0)) {
    ...
    grad_to_add += (k==0      ? 0 : wz * (param[index]-param[index-1]));
    grad_to_add += (k==sz_k-1 ? 0 : wz * (param[index]-param[index+1]));
    grad_to_add += (j==0      ? 0 : wy * (param[index]-param[index-sz_k]));
    grad_to_add += (j==sz_j-1 ? 0 : wy * (param[index]-param[index+sz_k]));
    grad_to_add += (i==0      ? 0 : wz * (param[index]-param[index-sz_k*sz_j]));
    grad_to_add += (i==sz_i-1 ? 0 : wz * (param[index]-param[index+sz_k*sz_j]));
